...
const Page = () => {
  ...
  const systemsWayRef = useRef<HTMLDivElement>(null);
  const accomplishmentsRef = useRef<HTMLDivElement>(null);
  const leadershipRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    const sections = [
      { ref: philosophyRef, x: -100 },
      { ref: coreValuesRef, x: 100 },
      { ref: systemsWayRef, y: 100 },
      { ref: accomplishmentsRef, x: -100 },
      { ref: leadershipRef, y: 100 },
    ];

    sections.forEach(({ ref, x = 0, y = 0 }) => {
      if (ref.current) {
        gsap.fromTo(
          ref.current,
          { opacity: 0, x, y },
  ...
  return (
    <div className="text-white">
      <div className="h-[400px] bg-[#1E2235]">
        <div className="max-w-20xl mx-auto px-20 py-8 relative">
          <h1 className="pt-40 text-6xl font-light leading-snug">
            Enabling a <br /> <span className="block text-white">digital</span>
          </h1>
        </div>
      </div>
      ...
      {/* Philosophy Section */}
      <div ref={philosophyRef} className="bg-white text-black mt-6 pb-10 px-20 relative">
        <h2 className="text-6xl font-light">tomorrow</h2>
        <div className="mt-8">
          <Link
            href="/contact-us"
            className="inline-block bg-[#1E2235] text-white border border-white px-6 py-3 text-sm tracking-widest hover:bg-white hover:text-[#1E2235] transition"
          ...
          <section
            ...
            className="pt-10 flex flex-col lg:flex-row justify-between items-start gap-10"
          >
            <div className="lg:w-1/2">
              <div className="mb-4 text-sm">
                <span className="font-semibold tracking-wider">OUR PHILOSOPHY</span>
            ...
            {/* Animated Clouds */}
            <div id="clouds" className="relative z-0">
              {[1, 2, 3, 4, 5].map((n, i) => (
                <div key={i} className={`cloud absolute top-[${5 + i * 5}%] left-[${10 + i * 15}%] opacity-70`}>
                  <img
                    src={`https://www.systemsltd.com/themes/custom/sysltd/assets/images/company/cloud${n}.png`}
                    alt={`cloud${n}`}
                  />
                </div>
              ))}
            ...
            <div className="lg:w-1/2">
              <img src="/balloon.png" alt="Our Philosophy" className="w-full object-cover" />
            </div>
          </section>
        </div>
      </div>
      ...
      {/* Core Values Section */}
      <section ref={coreValuesRef} className="bg-[#1E2235] text-white px-6 py-16">
        <div className="text-sm mb-8">
          <span className="font-semibold tracking-wider">OUR CORE VALUES</span>
        </div>
        <h3 className="text-4xl font-light mb-12 max-w-4xl">
          We dedicatedly master the power of technology in bespoke ways for our
          customers, guided by three core values:
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10 text-center border-t border-gray-600 pt-20 pb-10">
          {[
            { title: "Integrity", desc: "Embody dependability, honesty, and transparency." },
            { title: "Innovation", desc: "Act courageously, experiment, and make bold decisions." },
            { title: "Dedication", desc: "Commit to tirelessly delivering precision and perfection." },
          ].map((val, i) => (
            <div key={i} className={`px-4 ${i === 1 ? 'border-l border-r border-gray-600' : ''}`}>
      ...
      >
        <h4 className="text-sm uppercase tracking-widest mb-6">THE SYSTEMS WAY</h4>
        <h2 className="text-3xl md:text-4xl max-w-4xl mx-auto font-light leading-snug mb-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 max-w-6xl mx-auto text-center">
          {[
            { title: "Optimize", text: "We leverage customer-centric and cutting-edge talent and technology for higher business efficiency." },
            { title: "Transform", text: "We reimagine processes and systems by offering holistic solutions for superior enterprise value." },
            { title: "Scale", text: "We enable future-ready enterprises with long-term growth in a state of perpetual reinvention." },
          ].map((col, i) => (
            <div key={i}>
      ...
      {/* Accomplishments */}
      <section id="accomplishment" ref={accomplishmentsRef} className="bg-white text-black py-20 px-6">
        <h2 className="text-4xl font-light text-center mb-12">
          Our accomplishments as a <br /> global leader
        </h2>
        <div className="overflow-x-auto scrollbar-hide">
          <div className="flex space-x-6 w-max px-2">
            {["banking-tech", "temenos", "ibs-award", "forbes", "inner-circle"].map((img, i) => (
              <div
                key={i}
                className="min-w-[220px] bg-white text-center shadow-md p-4 rounded-lg"
              >
                <img
                  src={`/images/${img}.png`}
                  alt={`Award ${i + 1}`}
                  className="mx-auto mb-4 h-20 object-contain"
                />
                <p className="text-sm text-gray-600">Award description {i + 1}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Leadership */}
      <section ref={leadershipRef} id="leadership" className="bg-[#292f45] text-white py-20 px-6">
        <div className="max-w-7xl mx-auto">
          <div className="mb-12">
            <h4 className="text-sm uppercase tracking-widest mb-4">OUR LEADERSHIP</h4>
            <h2 className="text-3xl md:text-4xl font-light leading-snug max-w-3xl">
          ...
          <div className="relative mt-10">
            <div id="leader-carousel" className="flex space-x-8 overflow-x-auto scrollbar-hide pl-10 pr-6">
              {[                                {
                                    name: "Asif Peer",
                                    title: "Group CEO & Managing Director",
                                    img: "/images/leader1.png",
                ...
                <div
                  key={i}
                  className="min-w-[250px] bg-white rounded-lg p-4 text-black text-center"
                >
                  <img
                    src={leader.img}
                    alt={`Leader ${i + 1}`}

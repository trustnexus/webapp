...
export default function AdminLogoPage() {
  ...
  const { data, isLoading, error } = useFetch<{ data: { url: string }[] }>(
    "logos",
    "/logos"
  );

  const fetchedLogos = data?.data || [];
  ...
  const createLogo = useCreate<FormData>("/logo/upload", "logo");

  const handleUpload = async () => {
    if (!logoFile) return;

    const formData = new FormData();
    formData.append("image", logoFile); // backend should expect "logo"
